...
package com.tglt.sagittarius.smartspace.weather.icons
...
import com.tglt.sagittarius.util.getColorAccent
...
class WeatherIconPackProviderImpl(
    ...
    override fun getIcon(which: WeatherIconManager.Icon, night: Boolean): Bitmap {
        ...
        else AppCompatResources.getDrawable(context, R.drawable.weather_none_available)
                ).apply {
    ...
    companion object {
        ...
        private fun entry(day: String, night: String? = null) = IconMapEntry(day, night)

        fun getResName(icon: WeatherIconManager.Icon, night: Boolean) =
            PREFIX + (MAP[icon]?.getEntry(night)
                ?: NA)
    }
